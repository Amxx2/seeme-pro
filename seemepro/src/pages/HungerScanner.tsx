...
const HungerScanner = () => {
    ...
    const processFile = async (file: File) => {
        ...
        try {
            const res = await analyzeHungerWithOpenAI(file, i18n.language);
            if (completed) return;
            completed = true;
    ...
    const start = async () => {
        try {
            ...
            mr.onstop = async () => {
                ...
                try {
                    const res = await analyzeHungerWithOpenAI(blob, i18n.language);
                    if (completed) return;
                    completed = true;
                    clearTimeout(timeoutId);
                    setResult(res);
                            ...
                            >
                                    ...
                                    <div className="flex items-center gap-3 mb-3">
                                        ...
                                        <div>
                                            <h4 className={`font-black text-lg ${result.level === 'HIGH' ? 'text-red-400' :
                                                ...
                                                مستوى الجوع: {result.level === 'HIGH' ? 'مرتفع جداً (متلهف)' : result.level === 'MODERATE' ? 'متوسط' : 'منخفض (شبعان)'}
                                            </h4>
                                            <p className="text-gray-400 text-xs">درجة الجوع للنجاح: {result.score}/100</p>
                                        </div>
                                    </div>
                                ...
                                {result.traits?.length > 0 && (
                                    <div className="rounded-2xl border border-white/10 bg-white/3 p-5">
                                        <h4 className="text-gray-300 font-black text-sm mb-4">📊 مؤشرات الشغف التفصيلية</h4>
